...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		LSCMenu.categories.Plates:addSubMenu("LICENSE", "License", "Plate designs.",true)
		...
		respray = LSCMenu.categories:addSubMenu("RESPRAY", "Respray", "Transforms vehicle appearance.",true)
			pcol = respray:addSubMenu("PRIMARY COLORS", "Primary color",  nil,true)
				...
				pcol:addSubMenu("UTILITY", "Utility", nil,true)
				for n, c in pairs(LSC_Config.prices.utility.colors) do
					local btn = pcol.Utility:addPurchase(c.name,LSC_Config.prices.utility.price)btn.colorindex = c.colorindex
					if btn.colorindex == myveh.color[1] then
						btn.purchased = true
					end
				end
				pcol:addSubMenu("WORN", "Worn", nil,true)
				for n, c in pairs(LSC_Config.prices.worn.colors) do
					local btn = pcol.Worn:addPurchase(c.name,LSC_Config.prices.worn.price)btn.colorindex = c.colorindex
					if btn.colorindex == myveh.color[1] then
						btn.purchased = true
					end
				end
				pcol:addSubMenu("PEARLESCENT", "Pearlescent", nil,true)
				for n, c in pairs(LSC_Config.prices.pearlescent.colors) do
					local btn = pcol.Pearlescent:addPurchase(c.name,LSC_Config.prices.pearlescent.price)btn.colorindex = c.colorindex
					if btn.colorindex == myveh.color[1] and myveh.extracolor[1] == myveh.color[2] then
						btn.purchased = true
					end
				end
			...
			scol = respray:addSubMenu("SECONDARY COLORS", "Secondary color", nil,true)
				...
				scol:addSubMenu("UTILITY", "Utility", nil,true)
				for n, c in pairs(LSC_Config.prices.utility2.colors) do
					local btn = scol.Utility:addPurchase(c.name,LSC_Config.prices.utility2.price)btn.colorindex = c.colorindex
					if btn.colorindex == myveh.color[2] then
						btn.purchased = true
					end
				end
				scol:addSubMenu("WORN", "Worn", nil,true)
				for n, c in pairs(LSC_Config.prices.worn2.colors) do
					local btn = scol.Worn:addPurchase(c.name,LSC_Config.prices.worn2.price)btn.colorindex = c.colorindex
					if btn.colorindex == myveh.color[2] then
						btn.purchased = true
					end
				end
				scol:addSubMenu("PEARLESCENT", "Pearlescent", nil,true)
				for n, c in pairs(LSC_Config.prices.pearlescent2.colors) do
					local btn = scol.Pearlescent:addPurchase(c.name,LSC_Config.prices.pearlescent2.price)btn.colorindex = c.colorindex
					if btn.colorindex == myveh.color[2] and myveh.extracolor[1] == btn.colorindex then
						btn.purchased = true
					end
				end
		...
		LSCMenu.categories:addSubMenu("WHEELS", "Wheels", "Custom rims, tires and colors.",true)
			wtype = LSCMenu.categories.Wheels:addSubMenu("WHEEL TYPE", "Wheel Type", "Custom rims in all styles and sizes.",true)
				if IsThisModelABike(GetEntityModel(veh)) then
					...
					customw = wtype:addSubMenu("BENNYS BESPOKE WHEELS", "Bennys Bespoke", nil,true)
						for n, w in pairs(LSC_Config.prices.bbwheels) do
							local btn = customw:addPurchase(w.name,w.price)btn.wtype = w.wtype btn.modtype = 23 btn.mod = w.mod
						end
					customw = wtype:addSubMenu("BENNYS ORIGINALS WHEELS", "Bennys Originals", nil,true)
						for n, w in pairs(LSC_Config.prices.bowheels) do
							local btn = customw:addPurchase(w.name,w.price)btn.wtype = w.wtype btn.modtype = 23 btn.mod = w.mod
						end
					highendw = wtype:addSubMenu("HIGHEND WHEELS", "Highend", nil,true)
						for n, w in pairs(LSC_Config.prices.highendwheels) do
							local btn = highendw:addPurchase(w.name,w.price)btn.wtype = w.wtype btn.modtype = 23 btn.mod = w.mod
						end
					...
					customw = wtype:addSubMenu("STREET WHEELS", "Street", nil,true)
						for n, w in pairs(LSC_Config.prices.streetwheels) do
							local btn = customw:addPurchase(w.name,w.price)btn.wtype = w.wtype btn.modtype = 23 btn.mod = w.mod
						end
					customw = wtype:addSubMenu("TRACK WHEELS", "Track", nil,true)
						for n, w in pairs(LSC_Config.prices.trackwheels) do
							local btn = customw:addPurchase(w.name,w.price)btn.wtype = w.wtype btn.modtype = 23 btn.mod = w.mod
						end
					customw = wtype:addSubMenu("RACE WHEELS", "Race", nil,true)
						for n, w in pairs(LSC_Config.prices.racewheels) do
							local btn = customw:addPurchase(w.name,w.price)btn.wtype = w.wtype btn.modtype = 23 btn.mod = w.mod
						end
		...
		m = LSCMenu.categories.Wheels:addSubMenu("WHEEL ACCESSORIES", "Tires", "Bulletproof tires and custom burnout smoke.",true)
		...
		AddMod(9,LSCMenu.categories,"SIDE PANEL", "Side Panel", "A selection of side panels.",true)
		AddMod(17,LSCMenu.categories,"TEST", "Test", "Test 1.",true)
		AddMod(19,LSCMenu.categories,"TEST", "Test", "Test 2.",true)
		AddMod(20,LSCMenu.categories,"TEST", "Test", "Test 3.",true)
		AddMod(21,LSCMenu.categories,"TEST", "Test", "Test 4.",true)
		AddMod(23,LSCMenu.categories,"TEST", "Test", "Test 5.",true)
		AddMod(24,LSCMenu.categories,"TEST", "Test", "Test 6.",true)
		AddMod(47,LSCMenu.categories,"TEST", "Test", "Test 7.",true)
									...
									if pos.locked then
										...
										if not LSC_Config.lock then
											if IsControlJustPressed(1,153) then
												...
												drawTxt("Press [~b~E~s~] to enter the ~b~Vehicle Workshop~s~.",4,1,0.5,0.8,1.0,255,255,255,255)
											end
										else
											drawTxt("~r~Locked, please wait for player to finish.",4,1,0.5,0.8,0.5,255,255,255,255)
										end
									else
										if IsControlJustPressed(1,153) then
											inside = true
											...
											drawTxt("Press [~b~E~s~] to enter the ~b~Vehicle Workshop~s~.",4,1,0.5,0.8,0.5,255,255,255,255)
										end
									end
								else
								...
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									...
									if pos.locked then
										...
										if not LSC_Config.lock then
											...
											drawTxt("~r~Locked, please wait for player to finish.",4,1,0.5,0.8,1.0,255,255,255,255)
										end
									else
										inside = true
										currentpos = pos
...
function LSCMenu:onSelectedIndexChanged(name, button)
	...
	if m == "chrome" or m ==  "classic" or m ==  "matte" or m ==  "metals" or m ==  "utility" or m ==  "worn" or m ==  "chameleon" then
	...
	elseif m == "metallic" or m == "pearlescent" then
		if p == "primary color" then
			SetVehicleColours(veh,button.colorindex,myveh.color[2])
...
AddEventHandler("LSC:buttonSelected", function(name, button, canpurchase)
	...
	if mname == "chrome" or mname ==  "classic" or mname ==  "matte" or mname ==  "metals" or mname ==  "utility" or mname ==  "worn" or mname ==  "chameleon" then
	...
	elseif mname == "metallic" or mname == "metallic" then
		if m.parent == "Primary color" then
			if button.name == "Stock" or button.purchased or CanPurchase(price, canpurchase)then
				myveh.color[1] = button.colorindex
	...
	elseif mname == "sport" or mname == "muscle" or mname == "lowrider" or mname == "back wheel" or mname == "front wheel" or mname == "highend" or mname == "custom" or mname == "suv" or mname == "offroad" or mname == "tuner" or mname == "street" or mname == "track" or mname == "bb" or mname == "bo" or mname == "race" then
		if button.purchased or CanPurchase(price, canpurchase) then
...
function LSCMenu:OnMenuChange(last,current)
	...
	elseif c == "front wheel" or c == "wheel accessories" or  c == "wheel color" or c == "sport" or c == "muscle" or c == "lowrider"  or c == "highend" or c == "custom" or c == "suv" or c == "offroad" or c == "tuner" or c == "street" or c == "track" or c == "bb" or c == "bo" or c == "race" then
...
function CheckPurchases(m)
	name = m.name:lower()
	if name == "chrome" or name ==  "classic" or name ==  "matte" or name ==  "metals" or name ==  "utility" or name ==  "worn" or name ==  "chameleon" then
		if m.parent == "Primary color" then
	...
	elseif name == "metallic" or name == "pearlescent" then
		if m.parent == "Primary color" then
			for i,b in pairs(m.buttons) do
				if b.purchased and b.colorindex ~= myveh.color[1] then
					if b.purchased ~= nil then b.purchased = false end
	...
	elseif name == "sport" or name == "muscle" or name == "lowrider" or name == "back wheel" or name == "front wheel" or name == "highend" or name == "custom" or name == "suv" or name == "offroad" or name == "tuner" or name == "street" or name == "track" or name == "bb" or name == "bo" or name == "race" then
		for i,b in pairs(m.buttons) do
